...
function filedepot_getUserOptions() {
  $retval = '';
  $query = db_query("SELECT u.uid, u.name,u.status FROM {users} u LEFT JOIN {users_roles} r on (u.uid=r.uid) WHERE u.status = 1 AND r.rid in (4,6) ORDER BY name");
  //$retval .= '<option value = "200">osdhfiosduf</option>';
...
function filedepot_getRoleOptions() {
  $retval = '';
  $query = db_query("SELECT r.rid, r.name FROM {role} r WHERE r.rid in (4,6)");
